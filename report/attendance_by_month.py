...
class report_custom(report_rml):
    ...
    def create_xml(self, cr, uid, ids, datas, context=None):
        ...
        if context is None:
            context = {}
        month = datetime(2014, 4, 1)
        emp_ids = 1
        user_xml = ['<month>%s</month>' % "mariela", '<year>%s</year>' % "1990"]
        if emp_ids:
            for emp in obj_emp.read(cr, uid, emp_ids, ['name']):
                ...
                while today.month == month.month:
                    ...
                    order by att.name
                    '''
                    cr.execute(sql, (today.strftime('%Y-%m-%d %H:%M:%S'), tomor.strftime('%Y-%m-%d %H:%M:%S'), 1))
        ...
        ''' % (header_xml,_('Consolidado'),'\n'.join(user_xml),date_xml)
        return xml

report_custom('report.consolidate_budget', 'hr.employee', '', 'addons/prueba/report/bymonth.xsl')
